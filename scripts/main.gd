...
func start_runner():
	...
	run_timer.start(clamp(40 - (Global.loop * 5), 5, 40))
	...
	speed_tw.tween_property($platform_spawn, "spawn_speed", 1, run_timer.wait_time - 1).set_trans(Tween.TRANS_CIRC).set_ease(Tween.EASE_IN_OUT)
